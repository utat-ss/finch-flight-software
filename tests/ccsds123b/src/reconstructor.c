...
void decode_gpo2(char in[32], int *j, int *k)
{
	if (strcmp(in, "<neg>") == 0) {
		*j = -1;
		*k = -1;
	...
	*j = (prefix_val << *k) | suffix_val;
	...
	if (*k == 0 && prefix_val == Umax && in[0] != '1') {
		...
		while (in[current_idx] == '0' || in[current_idx] == '1') {
			if (current_idx >= D) {
				break; // j is D bits long
			}
			current_idx++;
		}
		direct_j_len = current_idx;

		if (direct_j_len <= D) { // It should be D bits long
			int zeros_count = 0;
			...
			if (zeros_count == Umax && in[current_idx + zeros_count] == '\0') {
				 // It matches the direct encoding. Decode j directly.
				for (int i = 0; i < D; ++i) {
					if (in[i] == '1') {
						direct_j |= (1 << (D - 1 - i));
					}
				...
				*j = direct_j;
				*k = D; // k is not used in this case, but for consistency, we can set it to D
				return;
			}
		}
	}
