...
/* * THE BINARY SWAP: This massive array lives purely in Internal Flash (ROM).
 * It uses exactly 0 bytes of RAM.
 */
const uint8_t dummy_photo_binary[1048576] = { 0xAA }; 
...
int benchmark_1mb_burst_write(const struct device *spi, const struct device *gpio) {
    LOG_INF("======================================");
    LOG_INF("Starting Zero-RAM DMA 1 MB Benchmark...");
    ...
    /* --- STAGE 2: THE BURST WRITE (DMA via ROM) --- */
    LOG_INF("Stage 2: Burst writing 1MB binary using DMA...");
    uint32_t write_start = k_uptime_get_32();

    for (int i = 0; i < total_pages; i++) {
        uint32_t current_addr = FLASH_TEST_ADDR + (i * chunk_size);
        
        /* Point directly to the chunk inside the ROM binary */
        const uint8_t *photo_chunk = &dummy_photo_binary[i * chunk_size];

        int ret = spi_flash_write(spi, gpio, CS_PIN_0, current_addr, photo_chunk, chunk_size);
        if (ret != 0) {
    ...
    for (int i = 0; i < total_pages; i++) {
        ...
        /* Verify against the chunk in our ROM binary */
        const uint8_t *expected_chunk = &dummy_photo_binary[i * chunk_size];
        if (memcmp(expected_chunk, read_buffer, chunk_size) != 0) {
    ...
    LOG_INF("======================================");
    LOG_INF("1 MB DMA BENCHMARK COMPLETE!");
