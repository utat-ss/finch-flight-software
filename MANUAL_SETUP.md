...
#### 3. Set the Finch Flight Software Environment Variable
```sh
source ./finch-flight-software-env.sh
```

### Flashing a board from manual environment

We suppose that the board is available through USB.

To flash the board, run the `west flash` command. To select a different runner, use the `-r` argument.
