...
Every source file must begin with a comment block with our copyright, a high level description of the file's purpose, functionality and its role. If the file controls a specific component or peripheral, include the part number and a link to the component's datasheet for reference
...
int eps_distribution_init(void);
```
 ...
 *
 * Returns 0 on success and FINCH_IMG_PREDICT_FAIL on error.
 */
int predict_image(
	const vec3 * N,
...
Our code is structred into the main app and libraries. The main app contains the `main` function for startup and all other code is put into the `lib` directory. Suppose you are writing the ADCS library. The structure of your code would be as follows,
...
Most functions should return `0` on success or an error code, defined as a macro in its header file. Functions may also log a message on error.

```c
int housekeeping_monitor_telem(Subsystem sys)
{
    int ret;
