#include "encoder.h"
#include "predictor.h"
#include "global.h"

#include <stdio.h>
#include <stdint.h>
#include <string.h>
#include <math.h>
#include <zephyr/kernel.h>
#include <zephyr/sys/printk.h>

void encode_prediction(int prediction[IMG_Z][IMG_Y][IMG_X], char out[IMG_Z][IMG_Y][IMG_X][32])
{
    for (int z = 0; z < IMG_Z; ++z)
    {
        int kz = 0;
        int accumulator = 1.0 / pow(2, 7) * (3 * pow(2, kz + 6) - 49) * 2;
        int counter = 2;

        for (int y = 0; y < IMG_Y; ++y)
        {
            for (int x = 0; x < IMG_X; ++x)
            {
                if (y != 0 && x != 0)
                {
                    if (counter < pow(2, 4) - 1)
                        accumulator += prediction[z][y][x];
                    else if (counter == pow(2, 4) - 1 && x > 0)
                        accumulator = (accumulator + prediction[z][y][x - 1] + 1) / 2.0f;
                    else if (counter == pow(2, 4) - 1 && x == 0 && y > 0)
                        accumulator = (accumulator + prediction[z][y - 1][IMG_X - 1] + 1) / 2.0f;

                    if (counter < pow(2, 4) - 1)
                        counter++;
                    else
                        counter = (counter + 1) / 2.0f;
                }

                // [5.4.3.2.2] Sample adaptive entropy encoder
                int32_t j = prediction[z][y][x], k;

                if (2 * counter > accumulator + (int32_t)(49/pow(2, 7) * counter))
                {
                    k = 0;
                }
                else
                {
                    for (k = 1; k <= D - 2; ++k)
                    {
                        if (counter * pow(2, k) > accumulator + (int32_t)(49/pow(2, 7) * counter))
                        {
                            k -= 1;
                            break;
                        }
                    }
                }

                encode_gpo2(j, k, out[z][y][x]);
            }
        }
    }
}

void encode_gpo2(int j, int k, char out[32])
{
    if (j >= (1U << D) || k > D)
    {
        strcpy(out, "<neg>");
        return;
    }
    
    unsigned int prefix = j >> k;
    unsigned int suffix = j & ((1 << k) - 1);
    int index = 0;

    if (prefix < Umax)
    {
        for (int i = k - 1; i >= 0; --i)
            out[index++] = (suffix & (1 << i)) ? '1' : '0';

        out[index++] = '1';

        for (unsigned int i = 0; i < prefix; ++i)
            out[index++] = '0';
    }
    else
    {
        for (int i = D - 1; i >= 0; --i)
            out[index++] = (j & (1 << i)) ? '1' : '0';

        for (unsigned int i = 0; i < Umax; ++i)
            out[index++] = '0';
    }

    out[index] = '\0';
}
