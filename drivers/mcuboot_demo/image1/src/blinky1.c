...
{
    printk("Button pressed! Requesting upgrade to slow blinky...\n");

    // Request upgrade - use TEST mode so it reverts if not confirmed
...
void main(void)
{
    ...
    printk("Running fast blinky. Press button to swap.\n");

    while (1) {
        gpio_pin_toggle_dt(&led);
        k_msleep(200);  // Fast blink: 0.2 sec
    }
}
