...
while true; do
    if [ $# -eq 0 ]; then
        pyocd gdbserver --connect under-reset --allow-remote --pack ${G4Pack} --pack ${H7Pack}
    elif [ $# -eq 1 ]; then
        pyocd gdbserver --target $1 --connect under-reset --allow-remote --pack ${G4Pack} --pack ${H7Pack}
    fi
