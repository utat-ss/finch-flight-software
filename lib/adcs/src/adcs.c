...
static uint8_t const adcs_id_expected[] =  { 0x54, 0x41, 0x44, 0x31, 0x30, 0x32, 0x30, 0x35, 0x33, 0x44, 0x00, 0x00 };

/* ADCS is connected via USART2. */
static const struct device *const adcs_uart = DEVICE_DT_GET(DT_NODELABEL(usart2));

/* Global buffer to store ADCS RX bytes. */
#define ADCS_RX_BUF_SIZE 32
static uint8_t adcs_rx_buf[ADCS_RX_BUF_SIZE];
static volatile uint8_t adcs_rx_len;
...
adcs_rc_t adcs_get_id(uint8_t *id, uint8_t id_size)
{
	...
	while (k_sem_take(&adcs_rx_sem, timeout) == 0) {
		timeout = K_MSEC(500);
	}

	if (adcs_rx_len < (4U + id_size)) {
	...
	for (uint8_t i = 0; i < id_size; ++i, ++adcs_rx_i) {
		if (adcs_rx_buf[adcs_rx_i] != adcs_id_expected[i]) {
			...
			LOG_ERR("ADCS ID has unexpected value at position %u, 0x%02x", i, adcs_rx_buf[adcs_rx_i]);
			return ADCS_RC_ERR;
		}
